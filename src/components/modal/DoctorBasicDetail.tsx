...
import DoctorEvaluationManager from './DoctorEvaluationManager'; // Import new component
import DoctorSpecialtyManager from './DoctorSpecialtyManager';
const defaultImage = require("../../..//public/img/avatars/no_image01.png");

export interface DoctorBasicDetailProps extends PropsWithChildren {
  isOpen : boolean;
  doctorId? : string; // 추가: doctor_id로 직접 조회하기 위함
  DoctorData? : any; // 선택 사항으로 변경
  setCloseModal : () => void;
  onUpdateSuccess?: () => void;
  sourceName?: string;
}

function DoctorBasicDetail(props: DoctorBasicDetailProps) {

  const { isOpen, doctorId, DoctorData, setCloseModal, sourceName } = props;
  ...
  const [mainCurrentTab, setMainCurrentTab] = React.useState<string>('basicInfo'); 
  ...
  const loadData = React.useCallback(
    async(id: string) => {
      ...
      try {
        // 1. 의사 상세 정보 조회
        const detailRes: any = await DoctorService.getDoctorDetailInformation({
          doctorId: id
        });
        
        let targetData = detailRes?.data || DoctorData;

        if (targetData) {
          // API 필드명 매핑 (doctrname -> doctorname, depthname -> deptname)
          targetData.doctorname = targetData.doctorname || targetData.doctrname;
          targetData.deptname = targetData.deptname || targetData.depthname;
          targetData.rid_long = targetData.rid_long || targetData.doctor_id;

          let careerData: any = [];
          // dc_jsondata가 문자열로 오는지, 이미 객체인지 확인하여 처리
          const rawJsondata = targetData?.dc_jsondata || targetData?.jsondata;
          
          if (!functions.isEmpty(rawJsondata)) {
            try {
              const parsedData = typeof rawJsondata === 'string' ? JSON.parse(rawJsondata) : rawJsondata;
              careerData = functions.categorizeByDynamicType(parsedData);
            } catch (e) {
        ...
        // 2. 논문 목록 조회
        const paperRes: any = await DoctorService.getDoctorPaperList({
          doctorId: id
        });
        setDoctorPapers(paperRes?.data || []);
  ...
  React.useEffect(() => {
    if (isOpen) {
      const idToFetch = doctorId || DoctorData?.doctor_id;
      if (idToFetch) {
        loadData(idToFetch);
      } else if (DoctorData) {
        let careerData: any = [];
  ...
  const onHandleUpdateCareerInfo = async(data:any) => {
    ...
    try{
      const bodyData = {
        doctor_id : data?.rid_long,
        jsondata : data?.education.concat(data?.career).concat(data?.awards).concat(data?.publications).concat(data?.articles).concat(data?.books).concat(data?.conferences),
        education : data?.education,
        career : data?.career,
        etc : data?.awards.concat(data?.publications).concat(data?.articles).concat(data?.books).concat(data?.conferences),
      }
    ...
    return (
      <>
        <Tabs variant='enclosed' width={'100%'} isLazy lazyBehavior='keepMounted'>
          ...
          >
            <Tab onClick={()=>setMainCurrentTab('basicInfo')} _selected={{bg: tabSelectColor,color:skeletonColor}}>기본정보</Tab>
            {sourceName === undefined && (
              <>
                <Tab onClick={()=>setMainCurrentTab('careerInfo')} _selected={{bg: tabSelectColor,color:skeletonColor}}>경력정보</Tab>
                <Tab onClick={()=>setMainCurrentTab('specialty')} _selected={{bg: tabSelectColor,color:skeletonColor}}>세부진료분야</Tab>
                <Tab onClick={()=>setMainCurrentTab('evaluation')} _selected={{bg: tabSelectColor,color:skeletonColor}}>평가정보</Tab>
              </>
            )}
          </TabList>
          <TabPanels>
            <TabPanel paddingTop={5}>
              ...
              <Box display={( isAdmin  && sourceName === undefined )? 'flex' : 'none'} flexDirection={'row'} justifyContent={'center'}  width={'98%'} mt={5}>
                <Button 
                  colorScheme='blue' 
                  variant='solid' 
                  width={'200px'} 
            ...
            >
              <Tabs variant='enclosed' width={'100%'} mt={5}>
                <TabList
                  overflowY="hidden"
                  sx={{
                    //borderBottom: "0px",
            ...
            <TabPanel padding={0}>
              <DoctorSpecialtyManager
                doctorId={inputs?.doctor_id || inputs?.rid_long}
                textColor={textColor}
                borderColor={borderColor}
              />
            </TabPanel>
            <TabPanel padding={0}>
              <DoctorEvaluationManager
                doctorId={inputs?.doctor_id || inputs?.rid_long}
                textColor={textColor}
                borderColor={borderColor}
              />
