...
export interface DoctorDetailProps extends PropsWithChildren {
  ...
  DoctorData : any;
  setCloseModal : () => void;
  onUpdateSuccess?: () => void; // 추가
  sourceName?: string; // 출처를 나타내는 prop 추가
}

function DoctorDetail(props: DoctorDetailProps) {

  const { isOpen, DoctorData, setCloseModal, sourceName } = props;
  ...
  const [mainCurrentTab, setMainCurrentTab] = React.useState<string>('basicInfo'); // Add mainCurrentTab state
