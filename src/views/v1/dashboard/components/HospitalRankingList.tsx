...
export default function HospitalRankingList({ rankingData }: HospitalRankingListProps) {
      ...
      <Flex align="center" mb="20px">
        ...
        {rankingData.length > initialDisplayLimit && ( // 데이터가 10개 초과일 때만 버튼 표시
          displayLimit === initialDisplayLimit ? (
            <Button onClick={handleShowMore} variant="link" colorScheme="brand" size="sm" color={buttonTextColor}>
              더보기
            </Button>
          ) : (
            <Button onClick={handleShowLess} variant="link" colorScheme="brand" size="sm" color={buttonTextColor}>
              간략히 보기
            </Button>
          )
        )}
